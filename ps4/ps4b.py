...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        result = {}
        for s in range(26):
            decrypted_message = super().apply_shift(s)
            ...
            for word in decrypted_words:
                if is_word(self.valid_words, word):
                    count += 1
            result[s] = count
        highest_count = max(result.values())
        shift = list(result.keys())[list(result.values()).index(highest_count)]
        return (shift, super().apply_shift(shift))
